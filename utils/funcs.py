...
import re, math, random, uuid, time, jsonpickle, json
...
class Funcs():
    def __init__(self, bot, cursor):
        ...
        self.colors = [
            'red', 'blue', 'green', 'gold', 'dark_blue', 'dark_gold', 'dark_green', 'dark_grey', 'dark_magenta',
            'dark_orange', 'dark_purple', 'dark_red', 'dark_teal', 'darker_grey', 'default', 'light_grey',
            'lighter_grey', 'magenta', 'orange', 'purple', 'teal'
        ]
        self.color_count = 0
    ...
    async def get_prefix(self, message):
        if self.bot.dev_mode:
            ...
            prefix = '>'
        prefix_set = False
        if not (isinstance(message.channel, discord.abc.PrivateChannel)) and (str(message.content).startswith(prefix + 'prefix') is True):
            ...
            sql_channel = sql_channel.format(message.guild.id, message.channel.id)
        ...
        if message.content.startswith(mention[0]):
            check = True
        else:
            check = await self.prefix_check(message.content, prefix, prefix_set)
        return ([prefix, mention[1], mention[0]], check)
    ...
    async def is_blacklisted(self, message):
        ...
        if message.author.id == self.bot.owner.id:
            return False
        global_blacklist_result = self.cursor.execute('SELECT * FROM `global_blacklist` WHERE user={0}'.format(
            message.author.id)).fetchall()
        if isinstance(message.channel, discord.abc.PrivateChannel):
            if (len(global_blacklist_result) != 0) and (message.author.id != bot.owner.id):
                return True
            return False
        muted_check_result = self.cursor.execute('SELECT * FROM `muted` WHERE server={0} AND id={1}'.format(
            message.guild.id, message.author.id)).fetchall()
        if (len(muted_check_result) != 0) and (message.guild.owner != message.author):
            return True
        guild_blacklist_result = self.cursor.execute('SELECT * FROM `blacklist` WHERE server={0} AND user={1}'.format(
            message.guild.id, message.author.id)).fetchall()
        channel_blacklist_result = self.cursor.execute(
            'SELECT * FROM `channel_blacklist` WHERE server={0} AND channel={1}'.format(
                message.guild.id, message.channel.id)).fetchall()
        if len(global_blacklist_result) != 0:
            return True
        elif len(guild_blacklist_result) != 0:
            return True
    ...
    async def command_check(self, message, command, prefix='.'):
        ...
        if message.channel.topic:
            ...
            topic_regex = re.compile(('((\\[|\\{)(\\+|\\-)?' + command_escape) + '(\\]|\\}))', re.I | re.S)
        ...
        try:
            ...
            pass
        if topic_match:
            await message.channel.send(':no_entry: **That command is disabled in this channel.**{0}'.format(
        ...
        for s in result:
            ...
            if s['type'] == 'server':
                if topic_match is False:
                    return False
                await message.channel.send(':no_entry: **That command is disabled on this server.**{0}'.format(
                    '\n`{0}command enable {1}` to enable the command.\n**Alternatively** place `[{1}]` in the channel topic or name.'.
                    format(prefix, command) if is_admin else ''))
                return True
            elif s['type'] == 'channel':
                if str(s['channel']) == str(message.channel.id):
                    await message.channel.send(':no_entry: **That command is disabled in this channel.**{0}'.format(
                        '\n`{0}command enable channel {1}` {2} to enable the command'.format(
                            prefix, command, message.channel.mention) if is_admin else ''))
                    return True
            elif s['type'] == 'role':
                for role in message.author.roles:
                    if str(role.id) == str(s['role']):
                        await message.channel.send(':no_entry: **That command is disabled for role: {1}**{0}'.format(
                            '\n`{0}command enable channel {1}` {2} to enable the command.'.format(
                                prefix, command, role.mention) if is_admin else ''), role.mention)
                        return True
            elif s['type'] == 'user':
    ...
    async def process_commands(self, message, c, p):
        ...
        ctx = Context(**tmp)
        del tmp
        if self.bot.get_command(c):
            command = self.bot.get_command(c)
            self.bot.dispatch('command', ctx)
            try:
                if command in (self.bot.get_command('repl'), self.bot.get_command('debug')):
                    await command.invoke(ctx)
                else:
                    with async_timeout.timeout(60):
                        await command.invoke(ctx)
            except (asyncio.TimeoutError, CancelledError, TimeoutError):
                try:
    ...
    async def queue_message(self, channel_id: str, msg):
        ...
        try:
            with aiohttp.Timeout(15):
                async with self.session.post('http://127.0.0.1:6969/queue', data=payload) as r:
                    return True
    ...
    async def run_process(self, code, response=False):
        try:
            ...
            create = loop.subprocess_exec((lambda: DataProtocol(exit_future)), *code, stdin=None, stderr=None)
