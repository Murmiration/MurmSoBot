...
class Changes(Cog):
    ...
    async def gist(self, ctx, log_type: int, idk, content: str):
        ...
        with aiohttp.ClientSession() as session:
            async with session.post('https://spit.mixtape.moe/api/create', data=payload) as r:
                url = await r.text()
                await ctx.send(':warning: Results too long (>= 1999)\n**Uploaded** to paste, URL: <{0}>'.format(url))
    ...
    async def names(self, ctx, user: str = None):
        ...
        try:
            if user is None:
                ...
                user = discord.Guild.get_member(ctx.guild, user_id=user)
                if user is None:
                    for guild in self.bot.guilds:
                        for member in guild.members:
                            if member.id == user:
                                user = member
                if user is None:
                    await ctx.send("Sorry, no user was found with that ID on any servers I'm on!")
                    return
            ...
            if len(result) == 0:
                await ctx.send(':no_entry: "{0}" does not have any name changes recorded!'.format(user.name))
                return
            results = ''
            names_added = []
            discrims_added = []
            ...
            if (user.nick != None) and (user.nick not in names_added):
                ...
                nick_added = False
            if (user.name != names_added[(-1)]) and (user.name != names_added[(-2)]) and (
                    user.name != names_added[(-3)]) and nick_added:
    ...
    @commands.command(aliases=['servernames', 'sname'])
    @commands.cooldown(1, 15)
    async def snames(self, ctx):
        ...
        try:
            ...
            result = self.cursor.execute(sql).fetchall()
            if len(result) == 0:
                await ctx.send(':no_entry: Server "{0}" does not have any name changes recorded!'.format(guild.name))
                return
            results = ''
            for s in result:
                name = s['name'].replace('@everyone', '@\u200beveryone').replace('@here', '@\u200bhere')
                if name == guild.name:
                    ...
                    results += '"{0}" `{1}`\n'.format(name, s['time'])
            final = '**Server Name Logs for** `{0}`\n'.format(guild.name) + results
            if len(final) >= 1999:
                results = results.replace('`', '').replace('*', '-')
                await self.gist(ctx, 2, guild.name, results)
            else:
                await ctx.send(final)
        except Exception as e:
            await ctx.send(e)
    ...
    async def on_member_update(self, before, after):
        ...
        if before.discriminator != after.discriminator:
            ...
            self.cursor.execute(sql, (before.id, '**Discriminator Change** ' + after.name, '0', time, before.guild.id,
                                      '`#{0}` => `#{1}`'.format(before.discriminator, after.discriminator)))
            self.cursor.commit()
        if before.name != after.name:
            ...
            if len(result) == 0:
                ...
                self.cursor.execute(sql, (before.id, before.name, '0', time, before.guild.id))
                self.cursor.commit()
            sql = 'INSERT INTO `names` (`id`, `name`, `nickname`, `time`, `server`) VALUES (%s, %s, %s, %s, %s)'
            self.cursor.execute(sql, (before.id, after.name, '0', time, before.guild.id))
            self.cursor.commit()
            ...
            if before.nick != None:
                ...
                if after.nick is None:
                    return
            sql = 'INSERT INTO `names` (`id`, `name`, `nickname`, `time`, `server`) VALUES (%s, %s, %s, %s, %s)'
            self.cursor.execute(sql, (before.id, after.nick, '1', time, before.guild.id))
            self.cursor.commit()
