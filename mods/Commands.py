...
class Commands(Cog):
    ...
    @commands.group(aliases=['setprefix', 'changeprefix'], invoke_without_command=True, no_pm=True)
    ...
    async def prefix(self, ctx, *, txt: str = None):
        ...
        if txt is None:
            ...
            sql_channel = sql_channel.format(ctx.guild.id, ctx.TextChannel.id)
            result = self.cursor.execute(sql).fetchall()
            result2 = self.cursor.execute(sql_channel).fetchall()
            if len(result) == 0:
                guild_prefix = '.'
            else:
                guild_prefix = result[0]['prefix']
            if len(result2) == 0:
                channel_prefix = None
            else:
        ...
        update_sql = update_sql.format(self.escape(txt), ctx.guild.id)
        check = 'SELECT server FROM `prefix` WHERE server={0}'
        check = check.format(ctx.guild.id)
        result = self.cursor.execute(check).fetchall()
        if len(result) == 0:
            self.cursor.execute(sql, (ctx.guild.id, txt, ctx.author.id))
            self.cursor.commit()
            await ctx.TextChannel.send(':white_check_mark: Set bot prefix to "{0}" for the server\n'.format(txt))
        else:
            self.cursor.execute(update_sql)
            self.cursor.commit()
            await ctx.TextChannel.send(':white_check_mark: Updated bot prefix to "{0}" for the server'.format(txt))

    @prefix.command(name='channel', no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def _prefix_channel(self, ctx, *, txt: str):
        ...
        update_sql = update_sql.format(self.escape(txt), ctx.guild.id, channel.id)
        check = 'SELECT * FROM `prefix_channel` WHERE server={0} AND channel={1}'
        check = check.format(ctx.guild.id, channel.id)
        result = self.cursor.execute(check).fetchall()
        if len(result) == 0:
            self.cursor.execute(sql, (ctx.guild.id, txt, channel.id, ctx.author.id))
            self.cursor.commit()
            await ctx.TextChannel.send(':white_check_mark: Set bot prefix to "{0}" for {1}'.format(txt, channel.mention))
        else:
            self.cursor.execute(update_sql)
            self.cursor.commit()
            await ctx.TextChannel.send(':white_check_mark: Updated bot prefix to "{0}" for {1}'.format(txt, channel.mention))

    @prefix.command(name='reset', no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def _prefix_reset(self, ctx, what: str = None, channel: discord.TextChannel = None):
        ...
        if (what is None) or (what == 'server'):
            ...
            if len(result) == 0:
                await ctx.TextChannel.send(
                    ':no_entry: Server does **not** reset a custom prefix set for any channel!\nMention the channel after "reset channel" for a specific channel.'
                )
                return
            else:
                self.cursor.execute(sql)
                self.cursor.commit()
                await ctx.TextChannel.send(':exclamation: Reset all channels custom prefixes!')
                return
        elif (what == 'all') or (what == 'everything'):
            sql = 'DELETE FROM `prefix_channel` WHERE server={0}'
            sql = sql.format(ctx.guild.id)
            sql2 = 'DELETE FROM `prefix` WHERE server={0}'
            sql2 = sql2.format(ctx.guild.id)
            self.cursor.execute(sql)
            self.cursor.execute(sql2)
            self.cursor.commit()
            await ctx.TextChannel.send(':warning: Reset all custom server prefix settings!')
            return
        else:
            await ctx.TextChannel.send(':no_entry: Invalid Option\nOptions: `server, channel, channels, all/everything`')
    ...
    async def command_toggle(self, t: str, ctx, cmd: str, user=None, msg=True):
        try:
            if cmd in self.good_commands:
                await ctx.TextChannel.send(':no_entry: You cannot disable command: `{0}`!'.format(
                    self.good_commands[self.good_commands.index(cmd)]))
                return
            if t == 'server':
                sql = "SELECT * FROM `command_blacklist` WHERE type='server' AND server={0} AND command={1}"
                sql = sql.format(ctx.guild.id, self.escape(cmd))
                result = self.cursor.execute(sql).fetchall()
                if len(result) == 0:
                    sql = 'INSERT INTO `command_blacklist` (`command`, `type`, `server`) VALUES (%s, %s, %s)'
                    self.cursor.execute(sql, (cmd, 'server', ctx.guild.id))
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':negative_squared_cross_mark: Disabled command `{0}`.'.format(cmd))
                else:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':white_check_mark: Enabled command `{0}`.'.format(cmd))
            elif t == 'channel':
                channel = user
                sql = "SELECT * FROM `command_blacklist` WHERE type='channel' AND server={0} AND channel={1} AND command={2}"
                sql = sql.format(ctx.guild.id, channel.id, self.escape(cmd))
                result = self.cursor.execute(sql).fetchall()
                if len(result) == 0:
                    sql = 'INSERT INTO `command_blacklist` (`command`, `type`, `server`, `channel`) VALUES (%s, %s, %s, %s)'
                    self.cursor.execute(sql, (cmd, 'channel', ctx.guild.id, channel.id))
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(
                            ':negative_squared_cross_mark: Disabled command `{0}` for channel {1}.'.format(
                                cmd, channel.mention))
                else:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':white_check_mark: Enabled command `{0}` for channel {1}.'.format(
                            cmd, channel.mention))
            elif t == 'user':
                sql = "SELECT * FROM `command_blacklist` WHERE type='user' AND server={0} AND user={1} AND command={2}"
                sql = sql.format(ctx.guild.id, user.id, self.escape(cmd))
                result = self.cursor.execute(sql).fetchall()
                if len(result) == 0:
                    sql = 'INSERT INTO `command_blacklist` (`command`, `type`, `server`, `user`) VALUES (%s, %s, %s, %s)'
                    self.cursor.execute(sql, (cmd, 'user', ctx.guild.id, user.id))
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(
                            ':negative_squared_cross_mark: Disabled command `{0}` for user `{1}`.'.format(cmd, user))
                else:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':white_check_mark: Enabled command `{0}` for user `{1}`.'.format(
                            cmd, user))
            elif t == 'role':
                role = user
                sql = "SELECT * FROM `command_blacklist` WHERE type='role' AND server={0} AND role={1} AND command={2}"
                sql = sql.format(ctx.guild.id, role.id, self.escape(cmd))
                result = self.cursor.execute(sql).fetchall()
                if len(result) == 0:
                    sql = 'INSERT INTO `command_blacklist` (`command`, `type`, `server`, `role`) VALUES (%s, %s, %s, %s)'
                    self.cursor.execute(sql, (cmd, 'role', ctx.guild.id, role.id))
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(
                            ':negative_squared_cross_mark: Disabled command `{0}` for role {1}.'.format(
                                cmd, role.mention))
                else:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':white_check_mark: Enabled command `{0}` for role {1}.'.format(
                            cmd, role.mention))
            elif t == 'global':
                ...
                if len(result) == 0:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':globe_with_meridians: Disabled command `{0}` globally.'.format(cmd))
                else:
                    ...
                    self.cursor.commit()
                    if msg:
                        await ctx.TextChannel.send(':white_check_mark: Enabled command `{0}` globally.'.format(cmd))
            else:
                return
        except Exception as e:
            await ctx.TextChannel.send(str(e))
    ...
    @commands.group(invoke_without_command=True, aliases=['commands', 'cmd'], no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def command(self, ctx, cmd: str):
        ...
        if cmd in self.bot.commands:
            ...
            await ctx.TextChannel.send(':no_entry: `Command does not exist.`')

    @command.command(name='toggle', aliases=['enable', 'disable'], invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    ...
    @command.command(name='user', aliases=['member'], invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    ...
    @command.command(name='role', aliases=['rank'], invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    ...
    @command.command(name='channel', invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def command_toggle_channel(self, ctx, cmd: str, chan: discord.TextChannel = None):
        'Toggle Command for a channel'
        if chan is None:
            chan = ctx.TextChannel
        if cmd in self.bot.commands:
            cmd = str(self.bot.commands[cmd])
    ...
    @command.command(name='global', invoke_without_command=True)
    @checks.is_owner()
    ...
    @command.group(name='module', invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def command_toggle_module(self, ctx, module: str, chan: discord.TextChannel = None):
        'Toggle a bot command module'
        try:
            mod = sys.modules['mods.{0}'.format(module)]
        except KeyError:
            modules = await self.get_modules()
            await ctx.TextChannel.send(':no_entry: Invalid Module\n**Modules**\n`{0}`'.format(', '.join(modules)))
            return
        if chan:
            count = await self.module_command_toggle(mod, 'channel', ctx)
        else:
            count = await self.module_command_toggle(mod, 'server', ctx)
        await ctx.TextChannel.send(':white_check_mark: Disabled **{0}** commands in module `{1}`.'.format(count, module))

    @command_toggle_module.command(name='list', invoke_without_command=True)
    async def command_toggle_module_list(self, ctx):
        modules = await self.get_modules()
        await ctx.TextChannel.send(':information_source: **Modules**\n`{0}`'.format(', '.join(modules)))

    @command.command(name='all', invoke_without_command=True, no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def command_toggle_all(self, ctx):
        ...
        self.cursor.execute(sql)
        self.cursor.commit()
        await ctx.TextChannel.send(':white_check_mark: Enabled **{0}** server command(s).'.format(count))

    @command.command(name='list', invoke_without_command=True, no_pm=True)
    async def command_list(self, ctx):
        sql = 'SELECT * FROM `command_blacklist` WHERE server={0} OR type="global"'
        sql = sql.format(ctx.guild.id)
        result = self.cursor.execute(sql).fetchall()
        if len(result) == 0:
            await ctx.TextChannel.send(':no_entry: Server does **not** have any commands blacklisted.')
            return
        msg = ''
        for s in result:
            if s['type'] == 'global':
        ...
        await ctx.TextChannel.send(':white_check_mark: **Commands Disabled**\n' + msg)
