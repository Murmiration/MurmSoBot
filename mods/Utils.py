...
class Utils(Cog):
	...
	async def status(self, ctx, *, status:str):
		...
		await ctx.send("ok, status changed to ``" + status + "``")
	...
	@checks.is_owner()
	async def dm(self, ctx, uid:int, *, content:str):
		try:
			user = await self.bot.get_user_info(str(uid))
	...
	async def load(self, ctx, *, module:str):
		...
		msg = await ctx.send("ok, loading `{0}`".format(mod))
		try:
			ctx.bot.load_extension(mod)
		except ImportError:
			await ctx.send(':warning: Module does not exist.')
			return
		except Exception as e:
	...
	async def unload(self, ctx, *, module:str):
		...
		if module in self.bot.cogs.keys():
			msg = await ctx.send("ok, unloading `{0}`".format(mod))
	...
	async def reload(self, ctx, *, module:str):
		mod = "mods.{0}".format(module)
		msg = await ctx.send("ok, reloading `{0}`".format(mod))
		try:
			ctx.bot.unload_extension(mod)
		except:
			await ctx.send(':warning: Module does not exist.')
		...
		try:
			...
			await ctx.send(code.format(e))
			return
		await msg.edit(content="ok, reloaded `{0}`".format(module))

	@commands.command()
	@checks.is_owner()
	async def update(self, ctx):
		await ctx.send("ok brb, killing my self")
	...
	# 		await ctx.send(code.format(result))
	# 	except execjs.ProgramError as e:
	# 		await ctx.send(code.format(type(e).__name__ + ': ' + str(e)))
	...
	# 	await ctx.send(cool.format(msg.replace("", "SERVER IP").replace("\n", "\n").replace("\"", "").replace("b'", "").replace("'", "")))

	@commands.command()
	async def iping(self, ctx, *, ip:str):
		msg = await self.bot.run_process(["ping", "-c", "4", "{0}".format(ip)], True)
		await ctx.send(cool.format(msg))

	@commands.command(no_pm=True, aliases=['colour'])
	...
	async def color(self, ctx, color:str=None):
		...
		try:
			...
			server_roles = list((map(str, ctx.message.guild.roles)))
			...
			if len(server_color_roles) != len(self.color_roles):
				await ctx.send(":warning: Server either does not have color roles or all the bots color roles\nAsk an Administrator to run the addcolors command\nor make sure you have all the roles in the colors command.")
				return
			if color is None:
				await ctx.send("You didn\'t input a color, here\'s a list\n```{0}```\nUse .color <color_name>".format(", ".join(self.color_roles)))
				return
			color = color.lower()
			role = discord.utils.find(lambda r: r.name.startswith(color), list(ctx.message.guild.roles))
			fix = list(set(' '.join(map(str, ctx.message.author.roles)).split(' ')) & set(self.color_roles))
			fix.append("aaa")
			erole = discord.utils.find(lambda r: r.name.startswith(fix[0]), list(ctx.message.guild.roles))
			if any([color == x for x in self.color_roles]) == False and color != "none":
				await ctx.send("You have input an invalid color, here\'s a list\n```{0}```\nUse .color <color_name>".format(", ".join(self.color_roles)))
				return
			if color != "none":
				if erole != None:
					await ctx.message.author.add_roles(erole)
					await asyncio.sleep(0.25)
					await ctx.message.author.add_roles(role)
					await ctx.send("Changing {0}'s color to `{1}`".format(ctx.message.author.mention, color))
				else:
					await asyncio.sleep(0.25)
					await ctx.message.author.add_roles(role)
					await ctx.send("Added color `{0}` to {1}".format(color, ctx.message.author.mention))
			elif color == "none" and any([roles == x for x in self.color_roles]):
				await asyncio.sleep(0.25)
				await ctx.message.author.remove_roles(erole)
				await ctx.send("Removed color from {0}".format(ctx.message.author.mention))
			else:
				if erole != None:
					await asyncio.sleep(0.25)
					await ctx.message.author.remove_roles(erole)
					...
					await ctx.send("You don't have a color to remove!")
		except Exception as e:
			print(e)
			await ctx.send(code.format(type(e).__name__ + ': ' + str(e)))
	...
	async def uncolor(self, ctx):
		...
		try:
			...
			if erole != None:
				await ctx.message.author.remove_roles(erole)
				await ctx.send("ok, removed color `{0}` from {1}".format(erole, ctx.message.author.mention))
			else:
				await ctx.send("You don't have a color to remove!")
		except Exception as e:
				await ctx.send(code.format(type(e).__name__ + ': ' + str(e)))
	...
	async def addcolor(self, ctx, name, color:discord.Colour):
		...
		try:
				if ctx.message.guild is None: return
				roles = list(map(str, ctx.message.guild.roles))
				for s in roles:
					if re.search(r'^' + s + r'$', name):
						await ctx.send("There's already a role with this name!")
						return
				await ctx.message.guild.create_role(permissions=permissions, name=name, color=color)
				await ctx.send("Added role with name `{0}` and color `{1}`".format(name, color))
		except Exception as e:
				await ctx.send(code.format(type(e).__name__ + ': ' + str(e)))

	@commands.command(aliases=['addcolours'], no_pm=True)
	...
	async def addcolors(self, ctx):
		...
		try:
			...
			server_color_roles = [i for e in server_roles for i in self.color_roles if e == i]
			if len(server_color_roles) == len(self.color_roles):
				await ctx.send(":warning: Server already has the color roles!")
				return
			...
			await ctx.message.guild.create_role(permissions=permissions, name='red', color=color)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='green', color=color2)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='purple', color=color3)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='blue', color=color4)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='orange', color=color5)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='black', color=color6)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='white', color=color7)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='cyan', color=color8)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='lime', color=color9)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='pink', color=color10)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='yellow', color=color11)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='lightred', color=color12)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='lavender', color=color13)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='salmon', color=color14)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='darkblue', color=color15)
			await asyncio.sleep(0.4)
			await ctx.message.guild.create_role(permissions=permissions, name='darkpurple', color=color16)
			await ctx.send("Added colors (16)\n```{0}```".format(", ".join(self.color_roles)))
	...
	async def mentions(self, ctx, max_messages:int=3000, idk=None):
		...
		if max_messages > 3000 and ctx.message.author.id != self.bot.owner.id:
			await ctx.send("2 many messages (<= 3000)")
			return
		if max_messages == 0:
			await ctx.send("Please input a number of messages to search through!\n Ex: `.mentions 20` searches through 20 messages if you were mentioned.")
		if max_messages > 0:
			await ctx.send("Sending results over PM `{0}`".format(ctx.message.author.name))
	...
	async def search(self, ctx, max_messages:int=None, *, text:str):
		...
		try:
			...
			if max_messages > 2000 and ctx.message.author.id != bot.owner.id:
				await ctx.send("2 many messages (<= 2000)")
				return
			if max_messages > 0:
				await ctx.send("Sending results over PM `{0}`".format(ctx.message.author))
			found_msgs = []
			async for message in self.bot.logs_from(ctx.message.channel, limit=max_messages):
				count = count - 1
	...
	async def on_message(self, message):
		if message.content[:2] == 's/' or message.content.startswith('sed/'):
			if message.channel.is_private is False:
				if message.guild.me.permissions_in(message.channel).send_messages is False:
					return
	...
	async def screenshot(self, ctx, *, url:str):
			try:
				...
				if os.path.getsize(location) <= 3150:
					await ctx.send("No screenshot was able to be taken\n Most likely cloudflare blocked me.")
					return
				await self.bot.delete_message(x)
				await ctx.send("ok, here\'s a screenshot of `{0}`".format(url))
	...
	async def admins(self, ctx):
		...
		for member in ctx.message.guild.members:
			...
			if member.status == discord.Status.online:
				...
				emtote = '<:vpOffline:212790005943369728> '
			if ctx.message.guild.owner == member:
		...
		if len(online) == 0:
			await ctx.send("No staff were found that are online!")
			return
		msg = ''
		for s in online:
			msg += '{0}\n'.format(s)
		await ctx.send("**Online Staff**\n"+msg)
	...
	async def rolecount(self, ctx, *, rolename:str):
		...
		for role in ctx.message.guild.roles:
			...
			if role.name.lower() in rolename or role.name.lower() == rolename:
				...
				break
		if rolee is None:
			await ctx.send(':no_entry: `Role not found.`')
			return
		count = 0
		for member in ctx.message.guild.members:
			if rolee in member.roles:
				count += 1
		await ctx.send('There is **{0}** members in the role `{1}`'.format(count, rolee.name))
		...
		try:
			...
			for key in google_api:
				...
				try:
					...
					pass
			if dead_keys:
				await ctx.send('Dead Keys (**{0}**/{1}): `{2}`'.format(len(dead_keys), len(google_api), ', '.join(dead_keys)))
			else:
				await ctx.send('woah, no dead keys!')
		except Exception as e:
			await ctx.send(e)
	...
	async def complain(self, ctx, *, message:str):
		try:
			...
			sql = 'INSERT INTO `feedback` (`shard`, `user`, `channel`) VALUES ({0}, {1}, {2})'.format(self.bot.shard_id, ctx.message.author.id, ctx.message.channel.id)
			self.cursor.execute(sql)
			self.cursor.commit()
			sql = 'SELECT `id` FROM `feedback` ORDER BY `id` DESC LIMIT 1'
			f_id = int(self.cursor.execute(sql).fetchall()[0]['id'])
			if isinstance(ctx.channel, discord.abc.PrivateChannel):
				await self.bot.queue_message(419156007856373770, "**Feedback #__{5}__**\nShard: `{6}`\nUser: **{0}** `<{0.id}>`\nChannel: `DMCHANNEL <{2.id}>`\nTime: __{3}__\nMessage:```{4}```".format(ctx.message.author, 'h', ctx.message.channel, ctx.message.created_at.strftime('%m/%d/%Y %H:%M:%S'), message, f_id, self.bot.shard_id))
			else:
				await self.bot.queue_message(419156007856373770, "**Feedback #__{5}__**\nShard: `{6}`\nUser: **{0}** `<{0.id}>`\nServer: `{1.name} <{1.id}>`\nChannel: `{2.name} <{2.id}>`\nTime: __{3}__\nMessage:```{4}```".format(ctx.message.author, ctx.message.guild, ctx.message.channel, ctx.message.created_at.strftime('%m/%d/%Y %H:%M:%S'), message, f_id, self.bot.shard_id))
			await ctx.send("ok, left feedback")
		except Exception as e:
			print(e)

	@complain.command(name='respond', invoke_without_command=True)
	@checks.is_owner()
	async def complain_respond(self, ctx, f_id, *, message:str):
		try:
			...
			if len(result) == 0:
				await ctx.send('Invalid Feedback ID!')
				return
			...
			try:
				...
				await ctx.send(':white_check_mark: Sent response!')
			except:
				await ctx.send(':warning: Error sending response.')
		except Exception as e:
			print(e)
