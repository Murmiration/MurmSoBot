...
class Logs(Cog):
    ...
    def remove_server(self, guild):
        ...
        remove_sql = remove_sql.format(guild.id)
        self.cursor.execute(remove_sql)
        self.cursor.commit()

    def remove_server_tracking(self, guild):
        remove_sql = 'DELETE FROM `tracking` WHERE server={0}'
        remove_sql = remove_sql.format(guild.id)
        self.cursor.execute(remove_sql)
        self.cursor.commit()

    @commands.group(aliases=['slogs', 'adminlogs'], no_pm=True, invoke_without_command=True)
    @checks.admin_or_perm(manage_server=True)
    async def guildlogs(self, ctx, channel: discord.TextChannel = None):
        ...
        if len(result) == 0:
            self.cursor.execute(sql, (ctx.guild.id, chan.id))
            self.cursor.commit()
            await ctx.send(':white_check_mark: Set Server Logs to {0.mention}'.format(chan))
        elif str(result[0]['channel']) == ctx.channel.id:
            self.cursor.execute(remove_sql)
            self.cursor.commit()
            await ctx.send(':negative_squared_cross_mark: Disabled Server Logs.')
        else:
            self.cursor.execute(update_sql)
            self.cursor.commit()
            await ctx.send(':white_check_mark: Updated Server Logs to {0.mention}'.format(chan))

    @guildlogs.command(name='disable', aliases=['off', 'clear', 'deactivate'], no_pm=True)
    async def guildlogs_disable(self, ctx):
        check = 'SELECT channel FROM `logs` WHERE server={0}'
        check = check.format(ctx.guild.id)
        result = self.cursor.execute(check).fetchall()
        if result:
            ...
            await ctx.send(':no_entry: Server does not have logs enabled!')

    @guildlogs.group(name='ignore', no_pm=True, invoke_without_command=True)
    @checks.admin_or_perm(manage_server=True)
    async def guildlogs_ignore(self, ctx, *users: str):
        if (not users):
            await ctx.send(':no_entry: Please input user mentions or id(s).')
            return
        members = []
        users = list(users)
        for u in users:
            if u.isdigit():
                ...
                if member and (member not in members):
                    ...
                    await ctx.send(':warning: `{0}` is an invalid id.'.format(u))
                    users.remove(u)
                    if len(users):
                        continue
                    return
        ...
        if s['ignore_users']:
            ...
            for u in members:
                if u.id in a:
                    ...
                    sql = sql.format(i, ctx.guild.id)
                    self.cursor.execute(sql)
                    self.cursor.commit()
                    await ctx.send(':negative_squared_cross_mark: Removed `{0}` from server log ignore.'.format(u))
                    members.remove(u)
                    if len(members):
                        continue
                    return
        sql = 'UPDATE `logs` SET ignore_users={0} WHERE server={1}'
        sql = sql.format(self.escape(', '.join([x.id for x in members])), ctx.guild.id)
        self.cursor.execute(sql)
        self.cursor.commit()
        await ctx.send(':white_check_mark: Ignoring `{0}` from server log.'.format(', '.join(
            [str(x) for x in members])))

    @guildlogs_ignore.command(name='avatar', no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def guildlogs_ignore_avatar(self, ctx, *users: str):
        if (not users):
            await ctx.send(':no_entry: Please input user mentions or id(s).')
            return
        members = []
        ...
        self.cursor.execute(sql)
        self.cursor.commit()
        await ctx.send(':white_check_mark: Ignoring `{0}` from avatar log.'.format(', '.join(
            [str(x) for x in members])))

    @guildlogs_ignore.group(name='global', no_pm=True, invoke_without_command=True)
    @checks.is_owner()
    async def guildlogs_ignore_global(self, ctx, *users: str):
        if (not users):
            await ctx.send(':no_entry: Please input user mentions or id(s).')
            return
        members = []
        ...
        for user in members:
            ...
            if result:
                ...
                await ctx.send(':white_check_mark: Added `{0}` to global log ignore.'.format(user))
            self.cursor.commit()

    @guildlogs_ignore_global.command(name='avatar', no_pm=True)
    @checks.is_owner()
    async def guildlogs_ignore_global_avatar(self, ctx, *users: str):
        if (not users):
            await ctx.send(':no_entry: Please input user mentions or id(s).')
            return
        members = []
        ...
        for user in members:
            ...
            if result:
                ...
                self.cursor.execute(sql)
                await ctx.send(
                    ':negative_squared_cross_mark: Removed `{0}` from global avatar log ignore.'.format(user))
            else:
                sql = 'INSERT INTO `global_log_ignore` (`user`, `avatar`) VALUES (%s, %s)'
                self.cursor.execute(sql, (user.id, 1))
                await ctx.send(':white_check_mark: Added `{0}` to global avatar log ignore.'.format(user))
            self.cursor.commit()

    @commands.group(invoke_without_command=True, aliases=['messagetracking', 'trackmessage'], no_pm=True)
    @checks.admin_or_perm(manage_server=True)
    async def track(self, ctx, *, txt: str):
        'Track messages in your server'
        if len(txt) > 500:
            await ctx.send(':no_entry: `Text too long (<= 500)`')
            return
        chan = ctx.channel
        sql = 'INSERT INTO `tracking` (`txt`, `server`, `channel`) VALUES (%s, %s, %s)'
        check = 'SELECT txt,id FROM `tracking` WHERE server={0} AND txt={1}'
        check = check.format(ctx.guild.id, self.escape(txt))
        ssss = 'SELECT * FROM `tracking` WHERE server={0}'
        ssss = ssss.format(ctx.guild.id)
        result = self.cursor.execute(check).fetchall()
        s_result = self.cursor.execute(ssss).fetchall()
        if (len(result) == 0) or (result[0]['txt'] != txt):
            if (len(s_result) != 0) and (len(s_result[0]) == 3):
                self.cursor.execute(sql, (txt, ctx.guild.id, s_result[0]['id']))
                self.cursor.commit()
                await ctx.send(':white_check_mark: Added "{0}" to tracking.'.format(txt))
            else:
                self.cursor.execute(sql, (txt, ctx.guild.id, chan.id))
                self.cursor.commit()
                await ctx.send(
                    ':white_check_mark: Set Channel to track all messages with "{1}"\nRun the channel command to change channel it logs in.'.
                    format(chan.mention, txt))
        else:
            await ctx.send(
                ':no_entry: `Tracking for text "{0}" already exists!`\nRemove using the remove command.'.format(txt))

    @track.command(name='channel', invoke_without_command=True, no_pm=True)
    async def _track_channel(self, ctx, chan: discord.TextChannel = None):
        if chan is None:
            chan = ctx.channel
        ...
        result = self.cursor.execute(check).fetchall()
        if len(result) == 0:
            await ctx.send(':no_entry: `This server is not tracking any messages!`')
        elif result[0]['id'] == chan.id:
            await ctx.send(
                ':no_entry: `Channel is already the tracking logs channel!`\nUse the remove all command to stop tracking.'
            )
        else:
            self.cursor.execute(update_sql)
            self.cursor.commit()
            await ctx.send(':white_check_mark: Updated text tracking channel to {0}'.format(chan.mention))

    @track.group(name='remove', invoke_without_command=True, no_pm=True)
    async def _track_remove(self, ctx, *, txt: str):
        remove_sql = 'DELETE FROM `tracking` WHERE server={0} AND txt={1}'
        remove_sql = remove_sql.format(ctx.guild.id, self.escape(txt))
        check = 'SELECT * FROM `tracking` WHERE server={0} AND txt={1}'
        check = check.format(ctx.guild.id, self.escape(txt))
        result = self.cursor.execute(check).fetchall()
        if len(result) == 0:
            await ctx.send(":no_entry: `That text isn't being tracked!`")
        else:
            self.cursor.execute(remove_sql)
            self.cursor.commit()
            await ctx.send(':x: Removed text from tracking.')

    @_track_remove.command(name='all', invoke_without_command=True, no_pm=True)
    async def _track_remove_all(self, ctx):
        ...
        result = self.cursor.execute(check).fetchall()
        if len(result) == 0:
            await ctx.send(':no_entry: `Server has not added any text to be tracked!`')
        else:
            self.cursor.execute(sql)
            self.cursor.commit()
            await ctx.send(':x: Reset/Removed all tracked text.')

    @track.command(name='list', invoke_without_command=True, no_pm=True)
    async def _track_list(self, ctx):
        sql = 'SELECT txt FROM `tracking` WHERE server={0}'
        sql = sql.format(ctx.guild.id)
        result = self.cursor.execute(sql).fetchall()
        if len(result) == 0:
            await ctx.send(':warning: `This server does not have any text being tracked!`')
        else:
            results = ''
            for s in result:
                results += '`{0}`\n'.format(s['txt'])
    ...
    @commands.command(aliases=['commanddelete', 'cmd_delete', 'command_delete'])
    @checks.admin_or_perm(manage_server=True)
    async def cmddelete(self, ctx):
        sql = 'SELECT * FROM `command_delete` WHERE server={0}'.format(ctx.guild.id)
        sql = sql.format(ctx.guild.id)
        result = self.cursor.execute(sql).fetchall()
        if result:
            ...
            self.cursor.execute(sql, ctx.guild.id)
            self.cursor.commit()
            await ctx.send(':white_check_mark: Enabled on Command Delete Messages.')

    async def is_ignored(self, message, **kwargs):
        ids = []
        if message is None:
            user = kwargs.pop('u')
            guild = kwargs.pop('server')
        else:
            user = message.author
            guild = message.guild
    ...
    async def on_command_message_delete(self, message):
        ...
        utc = int(time.time())
        if message.guild in spam_sent:
            sent_time = spam_sent[message.guild]
            sent_sec = int(sent_time) - int(utc)
            if (utc >= sent_time) or (sent_sec == 0):
                del spam_sent[message.guild]
            else:
                return
        spam_sent[message.guild] = utc + 5
        (command, prefix) = self.bot.command_messages[message]
        await message.channel.send(':warning: `{0}` **deleted command message**: {1}'.format(
            message.author, prefix + command))
    ...
    async def on_message(self, message):
        try:
            ...
            if len(result) == 0:
                return
            for s in result:
                if str(s['server']) != message.guild.id:
                    return
                if str(s['channel']) == message.channel.id:
                    return
                for x in message.content.lower().split():
                    if s['txt'].lower() == x:
                        msg = '`[{0}]` "{1}" was said **{2}** times in {3} by {4}'.format(
                            time.strftime('%I:%M:%S %p'), s['txt'], str(message.content.lower().count(
                                s['txt'].lower())), message.channel.mention, message.author)
                        await discord.Object(id=str(s['channel'])).send(msg)
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            if s:
                self.remove_guild_tracking(message.guild)
        ...
        try:
            ...
            if len(ctx.message.mentions):
                for s in ctx.message.mentions:
                    ctx.message.content = ctx.message.content.replace(s.mention, '@' + s.name).replace(
                        '<@!{0}>'.format(s.id), '@' + s.name)
            ...
            await self.truncate(
                target,
                ('`[{0}]` :exclamation: **Command Log**\n'.format(time.strftime('%I:%M:%S %p')) + cool.format(msg)) +
                msg2)
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(ctx.guild)
    ...
    async def on_message_delete(self, message):
        try:
            ...
            off_check = off_check.format(message.guild.id, message.author.id)
            ...
            for s in message.mentions:
                message.content = message.content.replace(s.mention, '@' + s.name).replace(
                    '<@!{0}>'.format(s.id), '@' + s.name)
            ...
            if len(message.attachments) != 0:
                ...
                if message in self.attachment_cache:
                    for attachment in self.attachment_cache[message]:
                        ...
                        if first:
                            await target.send(final, file=discord.File(attachment, filename=filename))
                            first = False
                        else:
                            await target.send(file=discord.File(attachment, filename=filename))
                    del self.attachment_cache[message]
                else:
                    await self.truncate(target, final)
            await self.truncate(target, final)
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(message.guild)
    ...
    async def on_message_edit(self, before, after):
        try:
            ...
            self.remove_guild(after.guild)

    async def on_guild_channel_delete(self, channel):
        try:
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            sql = sql.format(channel.guild.id)
            result = self.cursor.execute(sql).fetchall()
            if len(result) == 0:
                return
            ...
            if channel.id == chan:
                remove_sql = 'DELETE FROM `logs` WHERE server={0}'
                remove_sql = remove_sql.format(channel.guild.id)
                self.cursor.execute(remove_sql)
                self.cursor.commit()
                return
            guild = str(result[0]['server'])
            if channel.guild.id != guild:
                return
            a = channel.guild
            msg = '{0} Channel: {1} <{2}>\n'.format('Voice' if channel.type == discord.ChannelType.voice else 'Text',
                                                    channel.name, channel.id).replace("'", '')
            target = discord.Object(id=chan)
            await target.send(
                '`[{0}]` :x: **Channel Delete Log**\n'.format(time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(channel.guild)
    ...
    async def on_guild_channel_create(self, channel):
        try:
            ...
            msg = '{0} Channel: {1} <{2}>\n'.format('Voice' if channel.type == discord.ChannelType.voice else 'Text',
                                                    channel.name, channel.id).replace("'", '')
            target = discord.Object(id=chan)
            await target.send('`[{0}]` :white_check_mark: **Channel Create Log**\n'.format(
                time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(channel.guild)
    ...
    async def on_guild_channel_update(self, before, after):
        try:
            ...
            if before.name != after.name:
                msg += 'Name After: {0}\n'.format(after.name)
            if isinstance(before, discord.abc.PrivateChannel) == False:
                if isinstance(after, discord.abc.PrivateChannel) == True:
                    msg += 'Channel Private: Changed from False to True\n'
            elif isinstance(after, discord.abc.PrivateChannel) == False:
            ...
            target = discord.Object(id=channel)
            if msg:
                await target.send('`[{0}]` :warning: **Channel Edit Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                                  cool.format(('Channel Edited: {0} <{1}>\n'.format(before.name, before.id).replace(
    ...
    async def on_member_join(self, member):
        try:
            if member == self.bot.user:
                return
            check = await self.is_ignored(None, user=True, global_only=True, u=member, server=member.guild)
            if check:
                return
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            sql = sql.format(member.guild.id)
            result = self.cursor.execute(sql).fetchall()
            if len(result) == 0:
                return
            channel = str(result[0]['channel'])
            ...
            target = discord.Object(id=channel)
            await target.send(
                '`[{0}]` :inbox_tray: **Member Join Log**\n'.format(time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(member.guild)
    ...
    async def on_member_remove(self, member):
        try:
            ...
            if len(result) == 0:
                return
            if member.guild.id in self.banned_users:
                if member.id in self.banned_users[member.guild.id]:
                    return
            channel = str(result[0]['channel'])
            guild = str(result[0]['server'])
            if member.guild.id != guild:
            ...
            target = discord.Object(id=channel)
            await target.send('`[{0}]` :outbox_tray: **Member Leave/Kick Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                              cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(member.guild)
    ...
    async def on_member_ban(guild, self, member):
        try:
            ...
            if len(result) == 0:
                return
            if member.guild.id in self.banned_users:
                self.banned_users[member.guild.id] += member.id
            else:
                self.banned_users[member.guild.id] = [member.id]
            channel = str(result[0]['channel'])
            guild = str(result[0]['server'])
    ...
    async def on_member_unban(self, guild, member):
        try:
            if member == self.bot.user:
                return
            check = await self.is_ignored(None, user=True, global_only=True, u=member, server=guild)
            if check:
                return
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            ...
            if len(result) == 0:
                return
            channel = str(result[0]['channel'])
            guildr = str(result[0]['server'])
            if guild.id != guildr:
                return
            msg = 'Unbanned User: {0} <{0.id}>\n'.format(member)
            target = discord.Object(id=channel)
            await target.send('`[{0}]` :white_check_mark: **Member Unban Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                              cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
    ...
    async def on_member_update(self, before, after):
        try:
            if before == self.bot.user:
                return
            check = await self.is_ignored(None, user=True, server=before.guild, u=before)
            if check:
                return
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            sql = sql.format(before.guild.id)
            ...
            if before.avatar != after.avatar:
                if before.id == 135251434445733888:
                    return
                check = await self.is_ignored(None, avatar=True, server=before.guild, u=before)
                if (not check):
                    msg = 'User: {0} <{0.id}>\n'.format(after)
                    try:
                    ...
                    if before.avatar is None:
                        await target.send(
                            ':frame_photo: **New Avatar Log**\n' + cool.format(msg),
                            file=discord.File(after_avatar, filename='new_avatar.png'))
                        return
                    try:
                    ...
                    final.seek(0)
                    await target.send(
                        '`[{0}]` :frame_photo: **Avatar Change Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                        cool.format(msg),
                        file=discord.File(final, filename='avatar_change.png'))
            if before.name != after.name:
                msg = 'User Name Before: {0.name} <{0.id}>\n'.format(before)
                msg += 'User Name After: {0.name}\n'.format(after)
                await target.send('`[{0}]` :name_badge: **Name Change Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                                  cool.format(msg))
            if before.nick != None:
                if before.nick != after.nick:
                    ...
                    await target.send('`[{0}]` :warning: **Nickname Change Log**\n'.format(
                        time.strftime('%I:%M:%S %p')) + cool.format(msg))
            elif after.nick != None:
                msg = 'User: {0.name} <{0.id}>\n'.format(after)
                msg += 'Nickname: {0}\n'.format(after.nick)
                await target.send('`[{0}]` :warning: **Nickname Create Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                                  cool.format(msg))
            if before.roles != after.roles:
                ...
                for role in roles_removed:
                    if (not first_2):
                        msg += '\nRole{0} Removed: '.format('(s)' if len(roles_removed) > 1 else
                                                            '') if first_1 else 'Role{0} Removed: '.format(
                                                                '(s)' if len(roles_removed) > 1 else '')
                        first_2 = True
                    msg += str(role)
                await target.send(
                    '`[{0}]` :bangbang: **Roles Change Log**\n'.format(time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(after.guild)
        ...
        try:
            ...
            if before.icon != after.icon:
                ...
                if before.icon is None:
                    await target.send(
                        '`[{0}]` :frame_photo: **New Server Icon Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                        cool.format(msg),
                        file=discord.File(after_icon, filename='new_server_icon.png'))
                    return
                try:
                ...
                final.seek(0)
                await target.send(
                    '`[{0}]` :frame_photo: **Server Icon Change Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                    cool.format(msg),
                    file=discord.File(final, filename='server_icon_change.png'))
                await asyncio.sleep(2)
            if before.name != after.name:
                msg = 'Server Name Before: {0}\n'.format(before.name)
                msg += 'Server Name After: {0}\n'.format(after.name)
                await target.send('`[{0}]` :name_badge: **Server Name Change Log**\n'.format(
                    time.strftime('%I:%M:%S %p')) + cool.format(msg))
            if before.region != after.region:
                msg = 'Server Region Before: {0}\n'.format(str(before.region))
                msg += 'Server Region After: {0}\n'.format(str(after.region))
                await target.send('`[{0}]` :globe_with_meridians: **Server Region Change Log**\n'.format(
                    time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(after)

    async def on_guild_role_create(self, role):
        try:
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            sql = sql.format(role.guild.id)
            result = self.cursor.execute(sql).fetchall()
            if len(result) == 0:
                return
            channel = str(result[0]['channel'])
            guild = str(result[0]['server'])
            if role.guild.id != guild:
                return
            target = discord.Object(id=channel)
            msg = 'Server Role Created: {0}\n'.format(role.name)
            await target.send('`[{0}]` :bangbang: **Server Roles Create Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                              cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(role.guild)
    ...
    async def on_guild_role_delete(self, role):
        try:
            ...
            if role.guild.id != guild:
                return
            target = discord.Object(id=channel)
            r_b = ', '.join(map(str, role.guild.roles))
            msg = 'Server Role Deleted: {0}\n'.format(role.name)
            await target.send('`[{0}]` :bangbang: **Server Roles Delete Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                              cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(role.guild)
    ...
    async def on_guild_role_update(self, before, after):
        try:
            ...
            if before.permissions != after.permissions:
                ...
                if p1.manage_guild != p2.manage_guild:
                    msg += '--Manage Server--\nFrom {0} to {1}\n'.format(p1.manage_guild, p2.manage_guild)
                ...
                if p1.create_instant_invite != p2.create_instant_invite:
                    msg += '--Create Invites--\nFrom {0} to {1}\n'.format(p1.create_instant_invite,
                                                                          p2.create_instant_invite)
                ...
                if p1.move_members != p2.move_members:
                    msg += '--Move Members in Voice Channels--\nFrom {0} to {1}\n'.format(
                        p1.move_members, p2.move_members)
                ...
                if p1.read_message_history != p2.read_message_history:
                    msg += '--Read Message History--\nFrom {0} to {1}\n'.format(p1.read_message_history,
                                                                                p2.read_message_history)
                ...
                if p1.use_voice_activation != p2.use_voice_activation:
                    msg += '--Use Voice Activation--\nFrom {0} to {1}\n'.format(p1.use_voice_activation,
                                                                                p2.use_voice_activation)
            await target.send(
                '`[{0}]` :bangbang: **Roles Change Log**\n'.format(time.strftime('%I:%M:%S %p')) + cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(after.guild)

    async def on_voice_state_update(member, self, before, after):
        try:
            check = await self.is_ignored(None, user=True, u=before, server=before.guild)
            if check:
                return
            sql = 'SELECT server,channel FROM `logs` WHERE server={0}'
            sql = sql.format(before.guild.id)
            ...
            if msg:
                await target.send('`[{0}]` :bangbang: **Voice Channel Log**\n'.format(time.strftime('%I:%M:%S %p')) +
                                  cool.format(msg))
        except (discord.errors.Forbidden, discord.errors.NotFound, discord.errors.InvalidArgument):
            self.remove_guild(after.guild)
