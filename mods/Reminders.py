...
class Reminders(Cog):
    def __init__(self, bot):
        ...
        self.time_regex = re.compile(
            '(?:(?P<months>\\d+)mo)?(?:(?P<weeks>\\d+)w)?(?:(?P<days>\\d+)d)?(?:(?P<hours>\\d+)h)?(?:(?P<minutes>\\d+)m)?(?:(?P<seconds>\\d+)s)?'
        )
    ...
    async def remind(self, ctx, *, text: str = None):
        if text is None:
            await ctx.send(':warning: `Invalid Time Format`\n**Example:** `1h2m` (1 hour, 2 minutes)')
            return
        curr = ti.time()
        ...
        if time_result is False:
            ...
            await ctx.send(":warning: AFAIK, time can't be negative.")
            return
        else:
            epoch = int(curr) + time
        text = text if text != '' else None
        sql = 'INSERT INTO `reminders` (`user`, `time`, `message`) VALUES (%s, %s, %s)'
        self.cursor.execute(sql, (ctx.author.id, epoch, text))
        self.cursor.commit()
        await ctx.send(':white_check_mark: Reminder set for `{0}` seconds.'.format(time))
    ...
    async def remind_list(self, ctx):
        ...
        result = self.cursor.execute(sql).fetchall()
        if len(result) == 0:
            await ctx.send(":no_entry: You don't have any reminders set!")
            return
        reminders = []
        count = 0
    ...
    async def remind_remove(self, ctx, id: int):
        ...
        sql = 'DELETE FROM `reminders` WHERE user={0} AND time={1}'
        try:
            sql = sql.format(ctx.author.id, reminders[id])
        except:
            await ctx.send(':warning: `Invalid reminder id!`')
            return
        self.cursor.execute(sql)
        self.cursor.commit()
        await ctx.send(':x: Removed reminder **#{0}**'.format(id))
    ...
    async def remind_remove_all(self, ctx):
        ...
        self.cursor.execute(sql)
        self.cursor.commit()
        await ctx.send(':x: `Removed all reminders.`'.format(id))
