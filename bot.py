...
def init_funcs(bot):
    ...
    engine = create_engine(
        'mysql+pymysql://{0}:lolno@localhost/{1}?charset=utf8mb4'.format('murmsobot', db),
        encoding='utf8')
...
def prRed(prt):
    print('\x1b[91m {}\x1b[00m'.format(prt))


def prGreen(prt):
    print('\x1b[92m {}\x1b[00m'.format(prt))
    ...
    async def on_ready(self):
        if (not self.self_bot):
            ...
            if self.globals.already_ready or (downtime == 0):
                msg = '`[Shard {0}]` {1} has restarted, online since **{2}** for **{3}** second(s) | Current Time: **{4}**.'.format(
                    self.shard_id, self.user.mention, time_msg, downtime, current_time_msg)
            else:
                msg = '`[Shard {0}]` {1} is now online since **{2}** for **{3}** second(s) | Current Time: **{4}**\nServers: `{5}`'.format(
                    self.shard_id, self.user.mention, time_msg, downtime, current_time_msg, len(self.guilds))
            await self.queue_message(419162766465564683, msg)
        ...
        for cog in modules:
            try:
                ...
                await self.queue_message(419168973066600459,
        ...
        await self.change_presence(game=discord.Game(name="https://discord.gg/pgnDz4A"))

    async def on_message(self, message):
        self.last_message = message.created_at
        ...
        for prefix in prefixes:
            if message.content.lower().startswith(prefix) and check and (message.content.lower() != prefix):
                ...
                message_regex = re.compile((('(' + prefix_escape) + ')') + '[\\s]*(\\w+)(.*)', (re.I | re.X) | re.S)
                    ...
                    if message.channel in command_spam:
                        if command_spam[message.channel][1].count(command) >= 8:
                            ...
                            if (utc >= command_time) or (command_sec == 0):
                                ...
                                try:
                                    await message.channel.send('stop spamming `{0}`'.format(spam_command))
                                except:
                                    return
                                return
                        else:
...
#        embed.set_author(name='{0} <{0.id}>'.format(ctx.author), icon_url=ctx.author.avatar_url)
#        embed.add_field(name='Server', value='{0.name} <{0.id}>'.format(ctx.guild) if ctx.guild else 'Private Message')
#        embed.add_field(name='Channel', value='`{0.name}`'.format(ctx.channel))
#        embed.add_field(
#            name='Message',
#            value=ctx.message.clean_content + ' '.join([x['url'] for x in ctx.message.attachments]),
#            inline=False)
...
#             ctx.command.reset_cooldown(ctx)
    ...
    async def on_error(self, event, *args, **kwargs):
        ...
        await self.queue_message(419168973066600459, diff.format(fmt))

    async def on_command_error(ctx, e, self):
        try:
            if isinstance(e, commands.CommandOnCooldown):
                utc = int(time.time())
                cooldown_sent = self.globals.cooldown_sent
                if ctx.channel in cooldown_sent:
                    sent_time = cooldown_sent[ctx.channel]
                    sent_sec = int(sent_time) - int(utc)
                    if (utc >= sent_time) or (sent_sec == 0):
                        del cooldown_sent[ctx.channel]
                    else:
                        return
                else:
                    cooldown_sent[ctx.channel] = utc + 5
                    await ctx.channel.send(
                        ':no_entry: **Cooldown** `Cannot use again for another {:.2f} seconds.`'.format(e.retry_after))
                ...
                await ctx.channel.send(':no_entry: `No Permission`')
            elif isinstance(e, checks.No_Owner):
                await ctx.channel.send(':no_entry: `Bot Owner Only`')
            elif isinstance(e, checks.No_Mod):
                await ctx.channel.send(':no_entry: `Moderator or Above Only`')
            elif isinstance(e, checks.No_Admin):
                await ctx.channel.send(':no_entry: `Administrator Only`')
            elif isinstance(e, checks.No_Role):
                await ctx.channel.send(':no_entry: `No Custom Role or Specific Permission`')
            elif isinstance(e, checks.No_Sup):
                await ctx.channel.send(':no_entry: `Command only for "Murm\'s Servers`')
            elif isinstance(e, checks.No_GuildandPerm):
                await ctx.channel.send(':no_entry: `Server specific command or no permission`')
            elif isinstance(e, checks.Nsfw):
                await ctx.channel.send(
                    ':underage: `NSFW command, please add [nsfw] in your channel topic or move to a channel named nsfw!`'
                )
            elif isinstance(e, commands.CheckFailure):
                await ctx.channel.send(
                    ":warning: **Command check failed**\nCauses:\n `1.` Bot is missing `Administrator/Manage_roles` permission.\n `2.` You do not have proper permissions to run the command.\n `3.` The command is not to be used in PM's."
                )
            elif isinstance(e, commands.CommandInvokeError):
                if 'Forbidden' in str(e):
                    await ctx.channel.send(':warning: ' + code.format(str(e)))
                    ...
                    if 'status code: 400' in str(e):
                        try:
                            if (not ctx.message):
                                return
                            await self.truncate(ctx.message.content, ctx.channel)
                        except:
                            pass
                    elif 'status code: 413' in str(e):
                        try:
                            await ctx.channel.send(':warning: `Command failed to upload file: too large (>= 10 MB).`')
                        except:
                            pass
                else:
                    ...
                    embed.add_field(
                        name='Server', value='{0.name} <{0.id}>'.format(ctx.guild) if ctx.guild else 'Private Message')
                    ...
                    await self.queue_message(417358931741114369, embed)
            elif type(e).__name__ == 'NoPrivateMessage':
                try:
                    await ctx.channel.send(':warning: `Command disabled in Private Messaging.`')
                except:
                    pass
            elif (self.globals.command_errors == True) and isinstance(e, commands.CommandNotFound):
                await ctx.channel.send(':warning: Command `{0}` Not Found!'.format(ctx.command.name))
    ...
    async def on_guild_join(self, guild):
        ...
        embed.description = 'Shard ID: **{0}**'.format(self.shard_id)
        embed.set_author(name='{0} <{0.id}>'.format(guild.owner), icon_url=guild.owner.avatar_url)
        embed.add_field(name='Server', value='{0.name} <{0.id}>'.format(guild))
        embed.add_field(
            name='Members',
            value='**{0}**/{1}'.format(
                sum((1 for x in guild.members
                     if (x.status == discord.Status.online) or (x.status == discord.Status.idle))), len(guild.members)))
        embed.add_field(
            name='Channels',
            value='Text: `{0}`\nVoice: `{1}`\nTotal: **{2}**'.format(
                len(guild.text_channels),
                len(guild.voice_channels), len(guild.channels)))
        embed.color = discord.Color.green()
        embed.timestamp = datetime.datetime.now()
        await self.queue_message(419162200129798144, embed)

    async def on_guild_remove(self, guild):
        await self.wait_until_ready()
        sql = 'DELETE FROM `prefix_channel` WHERE server={0}'.format(guild.id)
        cursor.execute(sql)
        sql = 'DELETE FROM `prefix` WHERE server={0}'.format(guild.id)
        cursor.execute(sql)
        cursor.commit()
        embed = discord.Embed()
    ...
    async def on_resumed(self):
        ...
        msg = '`[Shard {0}]` {1} has now <@&211727010932719627> after being <@&211727098149076995> since **{2}** for **{3}** second(s) (Current Time: **{4}**)'.format(
            self.shard_id, self.user.mention, time_msg, downtime, current_time_msg)
        await self.queue_message(419162766465564683, msg)
